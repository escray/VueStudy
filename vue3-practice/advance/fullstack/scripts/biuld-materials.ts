...
async function main() {
  console.log('execute Material compiling...')
  const materialList = [
    {
  ...
  for (const opts of materialList) {
    console.log(`start compiling Material ${opts.dirName}`)
    const config = getBuildConfig(opts)
    const configAMD = getBuildAMDConfig(opts)
    await build(config)
    await build(configAMD)
    console.log(`push Material ${opts.dirName} 到模拟 CDN...`)
